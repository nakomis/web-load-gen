...
class StatusThread extends Thread {
    ...
    public StatusThread(Vector<Thread> threads, String label, boolean standardstatus, StatusNotify notify) {
        _threads = threads;
        _label = label;
    ...
    public void run() {
        ...
        do {
            ...
            _notify.update(interval/1000, totalops, curthroughput, Measurements.getMeasurements());
            if (totalops == 0) {
    ...
    public void run() {
        ...
        try {
            if (_dotransactions) {
                ...
                while (!stop && ((_opcount == 0) || (_opsdone < _opcount)) && !_workload.isStopRequested()) {
...
public class Client {
    ...
    public void start(StatusNotify notify, String[] args) {
        String dbname;
        ...
        if (status) {
            ...
            statusthread = new StatusThread(threads, label, standardstatus, notify);
            statusthread.start();
        }
